...
function e2eTestWorkflowPaths(rootPath: string): mod.OrchEngineWorkflowPaths {
  ...
  const inProcess: mod.OrchEngineWorkflowPaths["inProcess"] = {
    ...oeStorablePath(
      "support/assurance/ingestion-center-elt/drh/results-test-e2e",
    ),
...
// is executed using `deno task ahc-hrsn-drh-test-e2e` from repo root.
...
const args: mod.OrchEngineArgs = {
  ...
  referenceDataHome: path.join(Deno.cwd(), "src/ingestion-center-elt/reference-data"),
